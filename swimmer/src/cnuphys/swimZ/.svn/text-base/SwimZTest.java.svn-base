...
public class SwimZTest {
	...
	public static void main(String arg[]) {
		...
		System.out.println("Active Field Description: " + MagneticFields.getActiveFieldDescription());
...
//		testParabolicApproximation(numTest);
	...
	private static void testOldAdaptive(int numTimes) {
		...
		for (int i = 1; i <= numTimes; i++) {
			oldAdaptive(Q, xo/100, yo/100, zo/100, zf/100, p, theta, phi,
					i == numTimes);
		}
		double timePerSwim = ((double)(System.currentTimeMillis()-startTime))/numTimes;
	...
	private static void testOldUniform(int numTimes) {
		...
		long startTime = System.currentTimeMillis();
		for (int i = 1; i <= numTimes; i++) {
			oldUniform(Q, xo/100, yo/100, zo/100, zf/100, p, theta, phi,
					i == numTimes);
		}
		double timePerSwim = ((double)(System.currentTimeMillis()-startTime))/numTimes;
	...
	private static void partialReport(SwimZResult result, double timePerSwim, String name) {
		...
		System.out.println("p: " + pf
				+ " GeV/c" + " time/swim: " + timePerSwim + " ms");
	}
	
	private static void printVect(double v[], String s) {
		String out = String.format("%s [%-12.5f, %-12.5f, %-12.5f]", 
				s, v[0], v[1], v[2]);
	...
	private static void printSummary(String message, int nstep, double momentum, double Q[], double hdata[]) {
		...
		System.out.println(String.format("R = [%8.5f, %8.5f, %8.5f] |R| = %7.4f cm\nP = [%7.4e, %7.4e, %7.4e] |P| =  %9.6e GeV/c",
	 ...
	 * @param phi degrees
	 */
	private static void oldUniform(int Q, double xo, double yo, double zo, double zf, double p, 
			double theta, double phi, boolean printSumm) {

		
		// save about every 20th step
		double distanceBetweenSaves = 20 * oldUniformStepSize;

		SwimTrajectory traj = swimmer.swim(Q, xo, yo, zo, p,
				theta, phi, zf, oldAccuracy, 10.0, 10.0, oldUniformStepSize,
	 ...
	 * @param phi degrees
	 */
	private static void oldAdaptive(int Q, double xo, double yo, double zo, double zf, double p, 
			double theta, double phi, boolean printSumm) {
		...
		try {
			SwimTrajectory traj = swimmer.swim(Q, xo, yo, zo,
					p, theta, phi, zf, oldAccuracy, 10, 10, oldAdaptiveInitStepSize,
	...
	private static void hdataReport(double hdata[], double toCM) {
		String s = String.format("Min step: %-12.5e   Avg Step: %-12.5f   Max Step: %-12.5f cm", toCM*hdata[0], toCM*hdata[1], toCM*hdata[2]);
		System.out.println(s);
	}
