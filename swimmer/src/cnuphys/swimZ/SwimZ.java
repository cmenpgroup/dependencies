...
public class SwimZ {
	 ...
	 * @param relTolerance the absolute tolerances on each state variable [x, y, tx, ty] (q = const). So
	 * it is an array with four entries, like [1.0e-4 cm, 1.0e-4 cm, 1.0e-5, 1.0e05]
	 * @param hdata An array with three elements. Upon return it will have the min, average,
	 * and max step size (in that order).
	 * @return the swim result
	 * @throws SwimZException
	 */
	public SwimZResult adaptiveRK(int Q, double p, SwimZStateVector start, final double zf, double stepSize, double absError[], double hdata[]) throws SwimZException {
		if (start == null) {
		...
		int nStep = 0;
		try {
			nStep = rk4.adaptiveStepToTf(yo, start.z, zf, stepSize,
					z, y, deriv,
					_stopper, absError,
					hdata);
...
//			double hdata[]) throws RungeKuttaException {
	 ...
	 * @throws SwimZException
	 */
	public SwimZResult uniformRK4(int Q, double p, SwimZStateVector start, double zf, double stepSize) throws SwimZException {
		if (start == null) {
			throw new SwimZException("Null starting state vector.");
		}
	...
	public SwimZResult parabolicEstimate(int Q, double p, SwimZStateVector start, double zf, double stepSize) throws SwimZException {
		...
		//storage for results
		SwimZResult result = new SwimZResult(Q, p, start.z, zf, swimZrange.getNumStep()+1, 100);
		result.add(start);
...
//					double q) {
	...
	private SwimZResult straightLineResult(int Q, double p, SwimZStateVector start, double zf) {
		...
		SwimZStateVector v = new SwimZStateVector(x1, y1, zf, 
				start.tx, start.ty);
		result.add(v);
		return result;
	}
