 ...
 * A trajectory is a collection of state vectors. A state vector is
...
public class SwimTrajectory extends Vector<double []> {
	 ...
	 * @param charge the charge of the particle (-1 for electron, +1 for proton, etc.)
	 ...
	 * @param increment the size increment when the list needs to expand
	 */
	public SwimTrajectory(int charge, double xo, double yo, double zo,
			double momentum, double theta, double phi, int initialCapacity, int increment) {
		this(new GeneratedParticleRecord(charge, xo, yo, zo, momentum, theta, phi),
				initialCapacity, increment);
	}
	...
	/**
	 * Set the lund id. This is not needed for swimming,
	 * but is useful for ced or when MonteCarlo truth is known.
	...
	/**
	 * Get the lund id. This is not needed for swimming, and may
	 * be <code>null</code>.
	 * It is useful for ced or when MonteCarlo truth is known.
	 * return the Lund Id.
	 */
	...
	/**
	 * Get the average phi for this trajectory based on
	...
	/**
	 * Compute the integral Bdotdl. This will cause the state vector arrays to expand by
	 * two, becoming [x, y, z, px/p, py/p, pz/p, l, bdl] where
	 * the 7th entry l is cumulative pathlength in m and the eighth entry bdl is 
	 * the cumulative integral bdl in kG-m.
	 * @param field the field getter
	 */
