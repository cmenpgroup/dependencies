...
//		example9();
	...
	private static void example6() {
		...
		try {
			...
			terminalPlot(traj,
					"Pathlength (horizontal, m) vs. Int|Bxdl| (vertical, kg-m) [ADAPTIVE] ", 0);
	...
	private static void example7() {
		...
		try {
			SwimTrajectory traj = swimmer.swim(electron.getCharge(), xo, yo, zo,
					momentum, theta, phi, ztarget, accuracy, rmax, maxPathLength, stepSize,
					Swimmer.CLAS_Tolerance, hdata);
			double lastY[] = traj.lastElement();
			printSummary(
	...
	private static void example9() {
		...
		//_charge _x0 _y0 _z0 _pTot _theta _phi  z  accuracy _rMax, _maxPathLength, stepSize 
		//1 0.3336477532980491 -0.04022817961833376 2.3746720000000003 2.040380518389974E-8 170.99407849270204 173.86470849011025 2.292386 2.0E-5 7.0 5.0 5.0E-4
		...
		try {
			SwimTrajectory traj = swimmer.swim(charge, xo, yo, zo, pTot, theta, phi, zstop, zacc, rMax, maxPath, step, Swimmer.CLAS_Tolerance, hdata);
			double lastY[] = traj.lastElement();
			printSummary(
					"\nresult from adaptive stepsize method with storage and Z cutoff at " + zstop,
					traj.size(), momentum, lastY, hdata);
			terminalPlot(traj,
					"z (horizontal, cm) vs. x (vertical, cm) [ADAPTIVE] {STOP at Z=275}");
		} catch (RungeKuttaException e) {
