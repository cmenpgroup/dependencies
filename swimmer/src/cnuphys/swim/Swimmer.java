...
public class Swimmer {
	...
	/**
	 * In swimming routines that require a tolerance vector,
	 * this is a reasonable one to use for CLAS.
	 */
	 ...
	 * @param eps the baseline tolerance. The default is 1.0e-5. Probably
	 * should stay in the range 1e-10 (accurate but slow) to 1e-4 (inaccurate but fast)
	 */
	...
	//and returns a field in kG.This swim package works in SI (meters and Tesla)
	...
	/**
	 * Swims a Lund particle with a built it stopper for the maximum
	 * value of the radial coordinate. This is for
	 * the trajectory mode, where you want to cache steps along the
	 * path. Uses a fixed stepsize algorithm.
	 * 
	 ...
	 * @param stopper an optional object that can terminate the swimming
	 * based on some condition
	 * @param maxPathLength in meters. This determines the max number of steps based
	 * on the step size. If a stopper is used, the integration might terminate before
	 * all the steps are taken. A reasonable value for CLAS is 8. meters
	 * @param stepSize the uniform step size in meters.
	 * @param distanceBetweenSaves this distance is in meters. It should be bigger than 
	 * stepSize. It is approximately the distance between "saves" where the point
	 * is saved in a trajectory for later drawing.
	 * @return the trajectory of the particle
	 */
	public SwimTrajectory swim(int charge, 
        ...
        if (momentum < MINMOMENTUM) {
        	...
        	return  new SwimTrajectory(charge, xo, yo, zo,
    				momentum, theta, phi);      	
        }
	...
	/**
	 * Swims a Lund particle with a built in stopper for the maximum
	 * value of the radial coordinate. This is for
	 * the trajectory mode, where you want to cache steps along the
	 * path. Uses a fixed stepsize algorithm.
	 * 
	 ...
	 * @param maxPathLength in meters. This is used to compute the maximum number
	 * of integration steps. 
	 * @param stepSize either the initial stepsize or the constant step size based on
	 * the integration method. Units are meters.
	 * @param distanceBetweenSaves this should be bigger than stepSize. It is approximately
	 * the distance between "saves" where the point is saved in a trajectory for later
	 * drawing.
	 * @return the trajectory of the particle
	 */
	public SwimTrajectory swim(int charge, 
		...
		return swim(charge, xo, yo, zo,
				momentum, theta, phi,
				stopper, maxPathLength, 
				stepSize, distanceBetweenSaves);
	}
	
	
	/**
	 * Swims a charged particle. This is for
	 * the listener mode, where a callback is called for each advance
	 * of the integration Uses a fixed stepsize algorithm.
	 * 
	 * @param charge the charge: -1 for electron, 1 for proton, etc
	 * @param xo the x vertex position in meters
	 * @param yo the y vertex position in meters
	 ...
	 */
	public int swim(int charge, 
			double xo, double yo, double zo, 
			double momentum, double theta, double phi,
			IStopper stopper, 
			IRk4Listener listener,
			double maxPathLength,
			double stepSize) {		
	...
	/**
	 * Swims a particle with a built it stopper for the maximum
	 * value of the radial coordinate. This is for
	 * the trajectory mode, where you want to cache steps along the
	 * path. Uses a fixed stepsize algorithm.
	 * 
	 ...
	 * @param theta initial polar angle in degrees
	 * @param phi initial azimuthal angle in degrees
	 * @param fixedZ the fixed z value (meters) that terminates (or maxPathLength if reached first)
	...
	public SwimTrajectory swim(int charge, 
		...
		//our first attempt
		SwimTrajectory trajectory = swim(charge, xo, yo, zo,
				momentum, theta, phi,
				stopper, maxPathLength, 
				stepSize, distanceBetweenSaves);
		

		//if we stopped because of max radius, we are done (never reached target z)
		...
		while ((count < maxtry) && (del > accuracy)) {
			...
			SwimTrajectory addTraj = swim(charge, xo, yo, zo,
					momentum, theta, phi,
					stopper, maxPathLength, 
					stepSize, distanceBetweenSaves);
	...
	/**
	 * Swims a charged particle. This swims to a fixed z value. This is for
	 * the trajectory mode,  where you want to cache steps along the
	 ...
	 * @param fixedZ the fixed z value (meters) that terminates (or maxPathLength if reached first)
	 * @param accuracy the accuracy of the fixed z termination, in meters
	 * @param maxR the max radial coordinate
	 * @param maxPathLength in meters. This determines the max number of steps based
	 * on the step size. If a stopper is used, the integration might terminate before
	 * all the steps are taken. A reasonable value for CLAS is 8. meters
	 * @param stepSize the initial step size in meters.
	 * @param relTolerance the error tolerance as fractional diffs. Note it is a vector, the same
	 * dimension of the problem, e.g., 6 for [x,y,z,vx,vy,vz]. It might be something like
	 * {1.0e-10, 1.0e-10, 1.0e-10, 1.0e-8, 1.0e-8, 1.0e-8}
	 * @param hdata if not null, should be double[3]. Upon return, hdata[0] is the min stepsize
	 * used (m), hdata[1] is the average stepsize used (m), and hdata[2] is the max stepsize (m) used
	 * @return the trajectory of the particle
	...
	public SwimTrajectory swim(int charge, 
		...
		//if we stopped because of max path, we are done (never reached target z)
		...
		//have to deal with the fact that the hdata array will reset so save current values
...
//			momentum = traj.getFinalMomentum();
			...
			SwimTrajectory addTraj = swim(charge, xo, yo, zo, 
					momentum, theta, phi, stopper, maxPathLength, stepSize, relTolerance, hdata);
	...
	public SwimTrajectory swim(int charge, 
		...
		//integrate
		(new RungeKutta4()).adaptiveStep(uo, 0, maxPathLength, stepSize, s, u, deriv, stopper, 
	...
	/**
	 * Swims a charged particle for
	 * the listener mode, where a callback is called for each advance
	 * of the integration Uses an adaptive stepsize algorithm.
	 * 
	 * @param charge the charge: -1 for electron, 1 for proton, etc
	 * @param xo the x vertex position in meters
	 * @param yo the y vertex position in meters
	 ...
	 * dimension of the problem, e.g., 6 for [x,y,z,vx,vy,vz]. It might be something like
	 * {1.0e-10, 1.0e-10, 1.0e-10, 1.0e-8, 1.0e-8, 1.0e-8}
	 * @param hdata if not null, should be double[3]. Upon return, hdata[0] (m) is the min stepsize
	 * used, hdata[1] (m) is the average stepsize used, and hdata[2] (m) is the max stepsize used
	 * @return the total number of steps taken
	 * @throws RungeKuttaException 
	 */
	public int swim(int charge, 
	 ...
	 * @param hdata if not null, should be double[3]. Upon return, hdata[0] (m) is the min stepsize
	 * used, hdata[1] (m) is the average stepsize used, and hdata[2] (m) is the max stepsize used
	 * Swims a Lund particle. This is for
	 * the trajectory mode,  where you want to cache steps along the
	 * path. Uses an adaptive stepsize algorithm.
	 * @throws RungeKuttaException 
	 */
	public SwimTrajectory swim(int charge, 
		...
		//integrate
		(new RungeKutta4()).adaptiveStep(uo, 0, maxPathLength, stepSize, t, y, deriv, stopper, 
...
//		int ntotal = (int) (maxPathLength / stepSize); // number steps
	...
	public int swim(int charge, double xo, double yo, double zo,
		...
		return swim(charge, xo, yo, zo, momentum, 
				theta, phi, stopper, listener, maxPathLength, stepSize);
	}

	//create a straight line trajectory with just two points
	private SwimTrajectory straightLineTrajectory(GeneratedParticleRecord genPartRec,
			 double pathLen) {
	...
	private SwimTrajectory straightLineTrajectoryFixedZ(GeneratedParticleRecord genPartRec,
			 double zf) {
	...
	*/
	public static SwimTrajectory swimBackwardsToVertex(int q, double xo, double yo, double zo, double px, double py, double pz) {
		...
		try {
			traj = swimmer.swim(q, xo, yo, zo,
					p, theta, phi, ztarget, accuracy, 10, 10, stepSize,
