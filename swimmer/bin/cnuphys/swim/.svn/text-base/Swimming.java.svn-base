...
//				clearTrajectories();
	...
	private static void timeTest(int numTraj, long seed) {
		...
		for (double tolerance : tolerances) {
			...
			System.err.println("tolerance: " + tolerance + "m. Time per traj: " + timePerTraj / 100000 + "mS");
		}
	}
		...
		for (double tolerance : tolerances) {
			...
			for (int i = 0; i < numTraj; i++) {
				...
				try {
					...
					//get reference
					SwimTrajectory traj = getSwimmer().swim(charge, 0, 0, 0,
							momentum, theta, phi, ztarget, zacc, rmax, maxPathLength, stepSize,
							Swimmer.CLAS_Tolerance, hdata);
					double[] refLastY = traj.lastElement();

					//System.err.println("REF XYZ: (" + refLastY[0] + ", " + refLastY[1] + ", " + refLastY[2] + ")");
					
					
					traj = getSwimmer().swim(charge, 0, 0, 0,
							momentum, theta, phi, ztarget, zacc, rmax, maxPathLength, stepSize,
							clas_Tolerance, hdata);

					double[] lastY = traj.lastElement();
					//System.err.println("XYZ: (" + lastY[0] + ", " + lastY[1] + ", " + lastY[2] + ")");
			...
			System.err.println("tolerance: " + tolerance + " worst dev: " + worstDev + " mm  avg dev: " + avgDev + " mm");
		}
	}
	...
	private static void timeTest2(int numTraj, long seed) {
		...
		for (double zacc : zaccs) {
			...
			System.err.println("zacc: " + zacc + "m. Time per traj: " + timePerTraj / 100000 + "mS");
		}
	}
