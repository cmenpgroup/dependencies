...
/**
 * Static methods for Runge-Kutta 4 integration, including a constant
 * stepsize method and an adaptive stepsize method.
 * 
 * @author heddle
 * 
 */
...
//	}
	 ...
	 * does not store the results but instead uses an IRk4Listener to notify
	 ...
	 *            (actually 999).
	 * @param t will filled with the locations of t--should have the exact same
	 *            large dimension as the second index of y--i.e., something like
	 *            1000.
	 * @param deriv the derivative computer (interface). This is where the problem specificity resides.
	 ...
	 */
	public int uniformStep(double yo[], double to, double tf, final double y[][],
			final double t[], IDerivative deriv, IStopper stopper) {
	...
	/**
	 * Integrator that uses the standard RK4 advance with a uniform step size. (i.e.,
	 * this does NOT use an adaptive step size.)
	 * 
	 ...
	 */
	public int uniformStep(double yo[], double to, double tf, double h, IDerivative deriv, 
			IStopper stopper, IRk4Listener listener) {
	...
	/**
	 * Integrator that uses the RungeKutta advance with a Butcher Tableau and constant
	 * stepsize. (i.e., this does NOT use an adaptive step size.)
	 * 
	 * This version uses an IRk4Listener to notify
	 * the listener that the next step has been advanced.
	 * 
	 ...
	 */
	public int uniformStep(double yo[], double to, double tf, double h, IDerivative deriv, 
			IStopper stopper, IRk4Listener listener, ButcherTableau tableau) {
	 ...
	 * used, hdata[1] is the average stepsize used, and hdata[2] is the max stepsize used
	 * @return the number of steps used.
	...
	public int adaptiveStepToTf(double yo[], double to, double tf, double h,
			...
			IStopper stopper, double relTolerance[],
...
//				System.err.println("next Step:   " + tNext + "  [" + yNext[0] + ", " + yNext[1] + "]");
			}
...
//			yNext[1] + ", " + yNext[2] + "] h = " + h);
	 ...
	 * @param stopper if not <code>null</code> will be used to exit the
	 *            integration early because some condition has been reached.
	 * @param listener listens for each step	 * @param tableau the Butcher Tableau
	 * @param relTolerance the error tolerance as fractional diffs. Note it is a vector, the same
	 * @param hdata if not null, should be double[3]. Upon return, hdata[0] is the min stepsize
	 * used, hdata[1] is the average stepsize used, and hdata[2] is the max stepsize used
	 * @return the number of steps used.
	 * @throws RungeKuttaException 
	 */
	public int adaptiveStepToTf(double yo[], double to, double tf, double h, IDerivative deriv, 
			IStopper stopper, IRk4Listener listener, double relTolerance[],
			double hdata[]) throws RungeKuttaException {
	 ...
	 * @throws RungeKuttaException 
	 */
	public int adaptiveStep(double yo[], double to, double tf, double h, IDerivative deriv, 
			IStopper stopper, IRk4Listener listener, ButcherTableau tableau, double relTolerance[],
			double hdata[]) throws RungeKuttaException {
...
//		ButcherTableauAdvance advancer = new ButcherTableauAdvance(tableau);
	 ...
	 * @param yscale scale the error against this array. It can be the approximate max value of each
	 * component of y, which gives you constant absolute errors, or it can be null in which case
	 * y will be used and you have constant relative error.
	 * @param hdata if not null, should be double[3]. Upon return, hdata[0] is the min stepsize
	 * used, hdata[1] is the average stepsize used, and hdata[2] is the max stepsize used
	 * @return the number of steps used.
	 * @throws RungeKuttaException 
	...
	public int adaptiveStep(double yo[], double to, double tf, double h,
			final List<Double> t, final List<double[]> y, IDerivative deriv, 
			IStopper stopper, ButcherTableau tableau, double eps, double yscale[],
			double hdata[]) throws RungeKuttaException {
	 ...
	 */
	public int adaptiveStep(double yo[], double to, double tf, double h, IDerivative deriv, 
			IStopper stopper, IRk4Listener listener, ButcherTableau tableau, double eps, double yscale[],
			double hdata[]) throws RungeKuttaException {
...
//		}
	 ...
	 * @return the number of steps used.
	 */
	private int driver(double yo[], double to, double tf, double h, IDerivative deriv, 
			IStopper stopper, IRk4Listener listener, IAdvance advancer) {
	 ...
	 * @param absError the absolute tolerance for eact of the state variables. Note it is a vector, the same
	 * @param hdata if not null, should be double[3]. Upon return, hdata[0] is the min stepsize
	 * used, hdata[1] is the average stepsize used, and hdata[2] is the max stepsize used
	 * @return the number of steps used.
	 * @throw(new RungeKuttaException("Step size too small in Runge Kutta driver"));
	 */
	private int driverToTf(double yo[], double to, double tf, double h, IDerivative deriv, 
			IStopper stopper, IRk4Listener listener, 
			IAdvance advancer, double absError[], double hdata[]) throws RungeKuttaException {
...
//		System.err.println("EXCEEDED MAX PATH: pl = " + t + " MAX: " + tf);
	...
	private int driver(double yo[], double to, double tf, double h, IDerivative deriv, 
			IStopper stopper, IRk4Listener listener, 
			IAdvance advancer, double relTolerance[], double hdata[]) throws RungeKuttaException {
...
//				System.err.println("error " + error[i] + "  reltol: " + relTolerance[i] + "   dec: " + decreaseStep);
				if (decreaseStep) {
					break;
				}
			}
...
//		System.err.println("EXCEEDED MAX PATH: pl = " + t + " MAX: " + tf);
	...
	/**
	 * Driver that uses the RungeKutta advance with an adaptive step size.
	 * This uses an desired absolute error relative to some scale
     * (of max values of the dependent variables)
	 * 
	 * This version uses an IRk4Listener to notify
	 * the listener that the next step has been advanced.
	...
	private int driver(double yo[], double to, double tf, double h, IDerivative deriv, 
			IStopper stopper, IRk4Listener listener, IAdvance advancer, 
			double eps, double yscale[], double hdata[]) throws RungeKuttaException{
