...
//	private double _oldMomentum = 0.17988;
	...
	// for labels
	private static final String RELGAMMA = "Relativistic "
			+ SMALL_GAMMA;
	private static final String RELBETA = "Relativistic "
			+ SMALL_BETA;
			...
			try {
				...
				try {
					if (_standardCutoff.isSelected()) {
						SwimTrajectory traj = swimmer.swim(lid.getCharge(), xo, yo, zo,
								momentum, theta, phi, stopper, maxPathLen, stepSize,
...
//						}
						...
						if (_swimZ.isSelected()) {
							...
							SwimZResult result  = sz.adaptiveRK(lid.getCharge(), momentum, start, ztarget, adaptiveInitStepSize, adaptiveAbsError, hdata);
							...
							double accuracy = Double.parseDouble(_accuracy
									.getText()) / 1.0e6;
							traj = swimmer.swim(lid.getCharge(),
									xo, yo, zo, momentum, theta, phi, ztarget,
									accuracy, rMax, maxPathLen, stepSize,
									Swimmer.CLAS_Tolerance, hdata);
							traj.setLundId(lid);
							double lastY[] = traj.lastElement();
							printSummary(
	...
	private static void printSummary(String message, int nstep, double momentum, double Q[], double hdata[]) {
		...
		System.out.println(String.format("R = [%9.6f, %9.6f, %9.6f] |R| = %9.6f m\nP = [%9.6e, %9.6e, %9.6e] |P| =  %9.6e GeV/c",
		...
		System.out.println(String.format("Swim backards use theta: %9.6f  phi: %9.6f", theta, phi));
	...
	private JPanel initConditionsPanel() {
		...
		box.add(labeledTextField(MOMENTUMMAG,
				_momentumTextField,
						"GeV/c", -1));
		...
		box
				.add(labeledTextField(SMALL_THETA, _theta,
						"deg", 20));
		box.add(Box.createVerticalStrut(5));
	...
	private JPanel cutoffPanel() {
		...
		box.add(Box.createVerticalStrut(5));
		box.add(Box.createVerticalStrut(5));
		box.add(labeledTextField("      Stopping Z", _fixedZ, "cm",
				-1));
		box.add(Box.createVerticalStrut(5));
		box.add(labeledTextField("        Accuracy", _accuracy, "microns",
				-1));
		box.add(Box.createVerticalStrut(5));
	...
	private JPanel energyPanel() {
		...
		box.add(Box.createVerticalStrut(5));
		box.add(labeledTextField(MASS, _massTextField, " GeV/c"
				+ SUPER2, -1));
		box.add(Box.createVerticalStrut(5));
