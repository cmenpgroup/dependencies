...
public final class Environment {
	...
	private Environment() {
		...
		if (pfile.exists()) {
			try {
			    _preferences = (Properties) SerialIO.serialRead(pfile.getPath());
			}
			catch (Exception e) {
				e.printStackTrace();
			}
	...
	/**
	 * Get the application name. This is the simple part of the name
	 * of the class with the main metho. That is, if the main method 
	 * is in com.yomama.yopapa.Dude, this returns "dude" (converts to
	 * lower case.) 
	...
	//this is used to recommend to non AWT threads to wait to call for an update
	...
	/**
	 * Get a File object representing the preferences file. No
	...
	/**
	 * Convenience method to get a Vector of strings as a single
	 * string in the preferences file. For example, it might be a
	 * Vector of recently visited files.
	 * @param key the key
	...
	/**
	 * Convenience method to save a Vector of strings as a single
	 * string in the preferences file. For example, it might be a
	 * Vector of recently visited files.
	 * @param key the key
	 * @param value the vector holding the strings
	...
	/**
	 * Set whether or not dragging is occurring. This cam be used to pause threads that
	 * might be affecting the screen.
