...
public class StartET {
	...
	/**
	 * Start an ET system. In java, and et system is essentially a server socket. This
	 * will use the default server port in EtConstants.serverPort.
	 * @param sizeInBytes should be big enough for most evio events. 
	 * Will still work if we put in an event bigger than this.
	 * @param numEvents  the number of ET events
	 */
	 ...
	 * Start an ET system. In java, and et system is essentially a server socket. This
	 * will use the default server port in EtConstants.serverPort.
	 * @param etname the name of the et system. This should be the full path to a
	...
	public void shutDown() {
		if (_system == null) {
			Log.getInstance().warning("Trying to shutdwon ET system but the system object is null.");
			return;
		}
