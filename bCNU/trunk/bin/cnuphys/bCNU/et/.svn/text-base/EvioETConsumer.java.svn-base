...
public class EvioETConsumer {
	...
	public EvioETConsumer(String etName, String hostAddress, int port) {
		...
		try {
			...
			Log.getInstance().warning("CedETConsumer constructor: " + e.getMessage());
			_etSystem = null;
			_etAttachment = null;
		} catch (IOException e) {
			Log.getInstance().warning("CedETConsumer constructor: " + e.getMessage());
	...
	public EvioEvent nextEvent() {
		...
		if (_etSystem == null) {
			Log.getInstance().warning("Called nextEvent in CedETConsumer with null ET system");
			return null;
		}
		if (_etAttachment == null) {
			Log.getInstance().warning("Called nextEvent in CedETConsumer with null ET attachment");
			return null;
		}
	...
	public void detach() {
		...
		if (_etSystem == null) {
			Log.getInstance().warning("Called detach in CedETConsumer with null ET system");
			return;
		}
		if (_etAttachment == null) {
			Log.getInstance().warning("Called detach in CedETConsumer with null ET attachment");
			return;
		}
