...
public class BaseView extends JInternalFrame {
	...
	public BaseView(Object... keyVals) {
		...
		boolean iconifiable = _attributes
		 ...
		 boolean maximizable =
		 _attributes.booleanValue(AttributeType.MAXIMIZABLE);
		...
		if (worldSystem != null) {
			...
			if (_container == null) {
				_container = new BaseContainer(this, worldSystem, _attributes
						.booleanValue(AttributeType.HEADSUP));
			}
	...
	/**
	 * Checks a boolean property. Returns <code>false</code> if the
	 * property is not found
	 * @param key the key matched to the boolean we are checking
	 * @return <code>true</code> if the key is found and the value is "true" (case insensitive)
	 */
	...
	/**
	 * Obtain the properties of this view that can be used
	...
	/**
	 * Get the view type. This is not guaranteed to be meaningful or unique. It is
	 * up to the application to set unique view types if it intends to use them.
	...
	/**
	 * Set the view type. This is not guaranteed to be meaningful or unique. It is
	 * up to the application to set unique view types if it intends to use them.
