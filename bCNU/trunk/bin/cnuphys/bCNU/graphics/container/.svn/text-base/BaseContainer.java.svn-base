...
public class BaseContainer extends JComponent implements IContainer,
		MouseListener, MouseMotionListener, MouseWheelListener, IDrawableListener {
...
//			Log.getInstance().info("Layer shown: " + layer.getName());
	...
	protected void setAffineTransforms() {
		...
		localToWorld = AffineTransform.getTranslateInstance(_worldSystem.getMinX(),
				_worldSystem.getMaxY());
		localToWorld.concatenate(AffineTransform.getScaleInstance(scaleX,
				-scaleY));
		localToWorld.concatenate(AffineTransform.getTranslateInstance(-bounds.x,
				-bounds.y));
	 ...
	 * container's active area. Often there are no insets. Sometimes they
	 * are used so that text can be written in the inset area, such as for plot view.
