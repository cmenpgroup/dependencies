...
public abstract class AItem implements IDrawable, IFeedbackProvider,
	...
	public void drawSelections(Graphics g, IContainer container) {
		...
		if (isRotatable()) {
			...
			if ((rp != null) && (rotateIcon != null)) {
				...
				g.drawImage(rotateIcon.getImage(), rp.x
						- rotateIcon.getIconHeight() / 2 + 1, rp.y
						- rotateIcon.getIconHeight() / 2 + 1, container
						.getComponent());
			}
		}
	 ...
	 * 
	 * @param enabled the new value of the flag. If the item is enabled, it can
	 ...
	 * 
	 * @param dirty the new value of the flag. If the item is dirty, the next
	 *            time it is drawn it must be drawn from scratch. Many items are
	...
	public void stopModification() {
		...
		case DRAG:
			_layer
					.notifyDrawableChangeListeners(this,
							DrawableChangeType.MOVED);
			break;
...
//		return _rightClickable && isLayerEnabled();
	 ...
	 * 
	 * @param associatedObject the associated data bject to set
	 */
	 ...
	 * 
	 * @param associatedObject the associated graphic object to set
	 */
	 ...
	 * 
	 * @param child the child item to remove, leaving it an orphan (but not
	...
	/**
	 * Convenience check to see whether the item's layer is enabled. If
	 * it is not, you should not be able to select this item (independent
	 * of item's local selectability)
