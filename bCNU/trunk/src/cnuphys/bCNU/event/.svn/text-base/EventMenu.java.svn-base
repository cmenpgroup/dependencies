...
public class EventMenu extends EventTreeMenu {
	 ...
	 * @param eventTreePanel the required EventTreePanel
	 ...
	 * @param menu the actual JMenu event menu object.
	 */
	...
	public static void menuAdditions() {
		...
		if (_menu == null) {
			Log.getInstance().warning("Tried to add items to the event menu but it has not been set.");
		}
		...
		fileRadioMenuItem = new JRadioButtonMenuItem("Events From Files", EventControl.getEventSourceType() == EventControl.EventSourceType.FILE);
		etRadioMenuItem = new JRadioButtonMenuItem("Events From ET", EventControl.getEventSourceType() == EventControl.EventSourceType.ET);
...
//		EventMenu.etRadioMenuItem.setIcon(icon);
	 ...
	 * @param gotoTF the goto text field
	 */
	 ...
	 * @param mi the next event menu item
	 */
	...
	public static void fixItems() {
		...
		if (_gotoTextField != null) {
			_gotoTextField.setEnabled(EventControl.isSourceFile() && EventControl.isNextOK());
		}
...
//			System.err.println("First four bytes: " + rb[0] + " "+ rb[1] + " "+ rb[2] + " "+ rb[3] + " ");
//			System.err.println("Last four bytes: " + rb[len-4] + " "+ rb[len-3] + " "+ rb[len-2] + " "+ rb[len-1] + " ");
			eventTreePanel.setEvent(event);
		} catch (IOException e) {
	...
	public EvioEvent prevEvent() {
		...
		try {
			try {
				System.err.println("Will try to go to event number: " + (evNum-1));
