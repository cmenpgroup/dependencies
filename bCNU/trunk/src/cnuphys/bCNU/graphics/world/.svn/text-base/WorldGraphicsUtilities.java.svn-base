...
public class WorldGraphicsUtilities {
	...
	public static void drawWorldLine(Graphics g, IContainer container,
		...
		drawWorldLine(g, container, x1, y1, x2, y2, style.getLineColor(), style
				.getLineWidth(), style.getLineStyle());
	}
	...
	public static void drawWorldRadArc(Graphics g, IContainer container,
		...
		drawWorldRadArc(g, container, xc, yc, rmin, rmax, startAngle,
				stopAngle, style.getFillColor(), style.getLineColor(), style
						.getLineWidth(), style.getLineStyle());
	}
	...
	public static void drawWorldRectangle(Graphics g, IContainer container,
		...
		drawWorldRectangle(g, container, worldRectangle, style.getFillColor(),
				style.getLineColor(), style.getLineWidth(), style
						.getLineStyle());
	}
	...
	public static void drawHighlightedRectangle(Graphics g,
			IContainer container, Rectangle2D.Double wr, Color color1, Color color2) {
	...
	public static void drawLundHit(Graphics g, IContainer container,
		...
		if (style == null) {
			drawWorldRectangle(g, container, bounds, defFillColor, defFillColor
					.darker(), 1, LineStyle.SOLID);
		}
		else {
			drawWorldRectangle(g, container, bounds, style.getFillColor(), style.getFillColor(),
					1, LineStyle.SOLID);
		}
	...
	public static Polygon drawPath2D(Graphics g, IContainer container,
		...
		return drawPath2D(g, container, path, style.getFillColor(), style
				.getLineColor(), style.getLineWidth(), style.getLineStyle(),
				closed);
	}
	 ...
	 */
	public static Point2D.Double intersection(Point2D.Double p1, Point2D.Double p2,
	 ...
	 */
	public static Point2D.Double[] getRadArcPoints(Point2D.Double wpc, Point2D.Double wp1,
			double arcAngle) {
	 ...
	 * 
	 * @param path the path in question
	 * @param wp the point array--assumes it is big enough and the points have already
	 *     been created.
	 */
	 ...
	 */
	public static Point2D.Double polygonIntersection(Point2D.Double p0, double azimuth,
			Path2D.Double path) {
	 ...
	 */
	public static void project(Point2D.Double focus, double distance, double angle,
			Point2D.Double wp) {
