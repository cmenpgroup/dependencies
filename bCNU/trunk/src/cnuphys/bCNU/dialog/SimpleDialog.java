...
public class SimpleDialog extends JDialog implements ActionListener {
	 ...
	 * <code>null</code>.
	 * 
	 * @param component the component to add (oftent a panel with other
	 *            components)
	...
	protected Component createSouthComponent() {
		if (_closeout != null) {
			...
			buttonPanel
					.setLayout(new BoxLayout(buttonPanel, BoxLayout.LINE_AXIS));
			buttonPanel
	 ...
	 * all buttons, not just the closout buttons, so it is safe only if no more
	 * than one button has the given label.
	 * @param label the label to search for.
	 * @return the first button among its components that has the given label, or <code>null</code>.
	 */
	...
	/**
	 * Enable or disable a button with a given label. Assumes no more than
