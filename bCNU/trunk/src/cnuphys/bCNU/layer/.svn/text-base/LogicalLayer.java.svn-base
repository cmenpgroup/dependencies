...
public class LogicalLayer extends DrawableList {
	...
	/**
	 * Clears all the items. Not as simple as it appears. The main gotcha
	 * is that items were probably added to the container's feedback control
	 * as feedback providers. If they are not removed they will continue to
	 * produce feedback (and will not be garbage collected) even if they are
	 * removed from the layer--which itself only ensures that they will not
	 * be drawn.
	 * @param container
	 */
...
/**
 * Get all the items into a vector
 * All the items on this layer
 */
