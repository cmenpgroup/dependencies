...
//		maxThumbRectangle.setBounds(curMax - iw + 1, ymid - PINEXTRA, iw, ih);
...
//				- fullRangeMaxStrW / 2 -1, ymid - 3 - PINEXTRA);
		TextUtilities.drawGhostText(g, fullRangeMaxString, delMin + trenchWidth
				- fullRangeMaxStrW / 2 -1, ymid + 1 + PINEXTRA + fm.getHeight());
	}
	...
	/**
	 * Check whether the given value is in the current range. It is an inclusive check.
	 * @param val the value to test.
	 * @return <code>true</code> if the value is equal to a limit or between the limits.
	 */
	 ...
	 * @param del the increment of the runTime value. It will be incremented 
	 * from the current range min time to the max time in steps of del.
	 * @param nonAWTRunnable will be called first in each step and executed in
	 * a non-AWT thread.
		...
		Runnable runnable = new Runnable() {
			...
			public void run() {
				...
				while (running) {
					...
					//are we still running? If so schedule the awtRunnable which should update
					if (running) {
						if (awtRunnable != null) {
							try {
