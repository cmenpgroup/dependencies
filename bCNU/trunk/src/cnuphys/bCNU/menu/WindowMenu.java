...
/**
 * This singleton class creates a JMenu for a primitive list of
 * all internal frames in the window.
 * 
...
public class WindowMenu extends JMenu {
	...
	public void addWindowToList(BaseView v)
	{
		...
		if(!windowList.contains(v))
		{
				...
				public void actionPerformed(ActionEvent e) {
					...
					for(int i = 0; i < windowList.size(); i++)
					{
						...
						if(title.equals(windowList.get(i).getTitle()))
						{
							...
							{
								Desktop.getInstance().getDesktopManager().activateFrame(windowList.get(i));
							}
			...
			//Make JMenuItem
			JMenuItem windowItem = MenuManager.addMenuItem(
					v.getTitle(), 
	...
	public void removeWindowFromList(BaseView v)
	{
		...
		MenuManager.getInstance().getMenu("Window").remove(menuItemList.get(index));
		windowList.remove(index);
		menuItemList.remove(index);
	}
	
	/**
	 * As asked, the following disables the button if its 
	 * respected window is already on top
	 * TODO not properly disabling, instead seems to make it invisible
	 */
	public void setDisabled(BaseView v)
	{
		...
		for(int i = 0; i < windowList.size(); i++)
		{
			MenuManager.getInstance().getMenu("Window").getItem(i).setEnabled(true);
		}
		int index = windowList.indexOf(v);
		MenuManager.getInstance().getMenu("Window").getItem(index).setEnabled(false);
	}
	...
	/**
	 * Singleton objects cannot be cloned, so we override
	 * clone to throw a CloneNotSupportedException.
	 */
	@Override
