...
public class SimpleLogPane extends TextPaneScrollPane {
	...
	private void append(Grade grade, String message) {
		if (styles == null) {
			...
			styles.put(Grade.INFO, createStyle(Color.black, "sansserif",
					INFOFONTSIZE, false, false));
			styles.put(Grade.CONFIG, createStyle(Color.blue, "sansserif",
					CONFIGFONTSIZE, false, false));
			styles.put(Grade.WARNING, createStyle(Color.orange, "monospaced",
					WARNINGFONTSIZE, false, true));
			styles.put(Grade.ERROR, createStyle(Color.red, "monospaced",
					ERRORFONTSIZE, false, true));
		}
