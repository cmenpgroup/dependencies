...
public class ETTestConsumer {
	...
	public ETTestConsumer(String etName,
		...
		try {
	        ...
	        // create station
	        EtStation stat = sys.createStation(statConfig, "CED", position, pposition);
            ...
            while (true) {
				...
				for (EtEvent mev : mevs) {
					...
					try {
						...
						comHandler = new EvioCompactStructureHandler(buf, DataType.BANK);						
						System.out.println(" --- Compact Structure Nodes --------");
						//try { //Removing this since there is really nothing to try in order for the try catch to be used for this portion of the code
	 ...
	 * @param stream any print stream, such as System.out
	 * @param node
	 */
	public static void writeNode(PrintStream stream, EvioNode node) {
		if (node == null) {
	        ...
	        builder.append(", pos = "); builder.append(node.getPosition());
	        builder.append(", dataPos = "); builder.append(node.getDataPosition());
	...
	private static boolean bankOfBanks(EvioNode node) {
		...
		boolean sBank = (structureType == DataType.BANK) || (structureType == DataType.ALSOBANK);
		boolean dBank = (dataType == DataType.BANK) || (dataType == DataType.ALSOBANK);
		return sBank && dBank;
	}
