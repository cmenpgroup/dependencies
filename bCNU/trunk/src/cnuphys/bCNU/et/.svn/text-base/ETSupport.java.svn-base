...
public class ETSupport {
	...
	public static String _localHost = Environment.getInstance().getHostAddress();
	
	static{
		if(Environment.getInstance().isWindows()) {
			_defaultName = Environment.getInstance().getTempDirectory() + "et_sys_0000";
		}
		else {
	...
	public static JMenu getETMenu() {
		...
		_startLocalItem = new JMenuItem("Start Local ET With evio File Producer...");
	...
	private static void handleConnect() {
		...
		if (dialog.getAnswer() == DialogUtilities.YES_RESPONSE) {
			Log.getInstance().info("Try to connect to ET at " + dialog.getIpAddress() + " on server port: " + dialog.getPort());
			
			
			if (_evioConsumer != null) {
				Log.getInstance().warning("Trying to make an ET connection but the shared cedConsumer is not null as expected.");
				_evioConsumer.detach();
			}
			_evioConsumer = new EvioETConsumer(dialog.getETName(), dialog.getIpAddress(), dialog.getPort());
	...
	private static void handleStartLocal() {
		...
		if (_fileProducer != null) {
			Log.getInstance().warning("Trying to create a local ET but the shared fileProducer is not null as expected.");
			return;
		}
	...
	private static void handleKillLocal() {
		...
		if (_procRecord != null) {
//			System.err.println("killing ET system process: " + _procRecord.getProcess());
			_procRecord.kill();
	...
	public static EvioEvent nextEvent() {
		if (_evioConsumer == null) {
			Log.getInstance().warning("Called nextEvent in ETSupport with null CedETConsumer");
			return null;
		}
		else {
			_runningEventNum++;
	...
	public static File openProducerFile() {
		_producerEvioFile = FileUtilities.openFile(FileUtilities.getDefaultDir(),
				"evio Files", EventMenu._extensions);
