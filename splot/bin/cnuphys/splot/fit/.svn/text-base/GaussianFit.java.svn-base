...
//			System.err.println("Trimming");
		...
		//and return the x values and y values of the peaks in descending order,
		// i.e. pp[0] is the x and y of the highest maximum. It will then use that info
...
//			System.err.println(pp[i].x + "   " + pp[i].y + "  " + cs.value(pp[i].x) + "  " + stats[1]);
			aa[j+1] = pp[i].x;
//			aa[j+2] = 1.0;
			aa[j+2] = stats[1]/2;
		}
	...
	public static double[] secondaryInitialGuess(Fit fit) {
		...
		//the data, which is usually good at fitting the peaks. It will then find
		//and return the x values and y values of the peaks in descending order,
		// i.e. pp[0] is the x and y of the highest maximum. It will then use that info
		//as a very good guess of the gaussian means ()
		double xx[] = fit.getX();
	 ...
	 * @return an array with the mean in the 0 index and
	 * standard deviation is the 1 index
	 */
