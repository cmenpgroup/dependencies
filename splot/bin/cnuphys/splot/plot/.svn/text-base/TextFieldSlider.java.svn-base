...
public abstract class TextFieldSlider extends JPanel implements ChangeListener {
	...
	public TextFieldSlider(int min, int max, int value, Font font,
			int minorTickSpacing, String labels[], int sliderWidth, int textFieldWidth, String borderTitle) {
		_defaultValue = value;
	...
	/**
	 * Convert a real value to the corresponding slider value. This
	 * allows, for example, to have a slider range of 0 to 100 mapped to a
	 * real-valued range that is not even necessarily linear.
	 * @param val the real value to convert.
	 * @return the corresponding slider value. (The slider is not automatically set to this value)
	 */
	...
	/**
	 * This is called when the value changes, with by the slider or by entering text into the 
	 * text field and hitting return.
	 */
