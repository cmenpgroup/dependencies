...
public class HistoData implements XmlPrintStreamWritable {
	...
	/**
	 * The data for a 1D histogram where the bin spacing
	 * is uniform.
	 * @param name the curve name of the histogram
	 * @param valMin the data min
	 * @param valMax the data max
	 * @param numBins the number of bins
	 * @param rebinLimit store raw data to here, then store binned data only, at
	 * which point rebinning is disabled;
	 */
	public HistoData(String name, double valMin, double valMax, int numBins, int rebinLimit) {
	 ...
	 * @return an array with the mean in the 0 index, 
	 * standard deviation is the 1 index, and rms in the 2 index
	 */
	...
	/**
	 * Get the number on entries in the histogram that
	 * were below the minimum value
	...
	/**
	 * Get the minimum "x" value. This is
	 * the minimum of the range being binned.
	...
	/**
	 * Get the minimum value. This is
	 * the maximum of the range being binned.
	 * @return the minimum "x" value
	 */
	...
	/**
	 * Get the minimum "y" value. The y axis corresponds
	 * to "counts", so this always returns 0
	 */
	...
	/**
	 * Get the maximum "y" value. The y axis corresponds
	 * to "counts", so this always returns the count
	 * of the bin with the most counts.
	 */
	...
	public void add(double value) {
		...
		if (_rebinEnabled) {
			if (_rawData.size() >= _rebinLimit) {
				System.err.println("Threshold exceeded, deleting raw data. Rebinning disabled");
				_rebinEnabled = false;
				_rawData = null;
			}
			else {
	...
	/**
	 * Get the bin for a given value. Will return the zero-based bin number
...
//			System.err.println("Fix negative bin " + bin);
	...
	public static void main(String arg[]) {
		...
		for (int i = 0; i < hd.getNumberBins(); i++) {
			System.err.println("bin: " + i + "  counts: " + hd.getCounts()[i] + "  mid val: " + hd.getBinMidValue(i));
		}
	...
	private void writeBasicData(XmlPrintStreamWriter writer) throws XMLStreamException {
	...
	private void writeData(XmlPrintStreamWriter writer) throws XMLStreamException {
