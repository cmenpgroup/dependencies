...
//	}
	...
	/**
	 * Sort a set of parallel arrays. The first one is the one
	 * that will end up sorted. Does nothing if the first array is sorted.
	 ...
	 * @return a double[4] containing, in order, the mean, min, max, and standard deviation
	 */
	...
	/**
	 * This deals with missing or bad error data. If the sigma array is null,
	 * it creates a new array and assigns a value of 1 to each error. It does the same 
	 * if the sigma array has all zero values. The real troubling case is when sigma has some 
	 * zero values. This will cause many fits to fail. So in that case it repleaces all zero
	 * values with the mean value. At that point the fit should be considered suspect.
