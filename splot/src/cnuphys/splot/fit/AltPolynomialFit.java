...
public class AltPolynomialFit extends Fitmrq implements IValueGetter {
	...
	/**
	 * Create an alternate polynomial fit that uses the nonlinear fitter
	 * rather than generalized least squares.
	 * @param fit the fit data
	 */
	public AltPolynomialFit(Fit fit) {
		super(fit.getX(), fit.getY(), fit.getSigmaY(), initialGuess(fit), createFPoly(fit.getPolynomialOrder()+1), fit.getTolerance());
