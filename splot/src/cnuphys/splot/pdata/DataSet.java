 ...
 * 
 */
public class DataSet extends DefaultTableModel implements XmlPrintStreamWritable {
	 ...
	 * @throws DataSetException When the number of columns is inconsistent with the type
	 */
	public DataSet(DataSetType type, String... colNames) throws DataSetException {
		...
		case XYXY: 
			//repeated x,y columns. Number of columns should be divisible by two
		...
		case XYEXYE: 
			...
			if ((colCount % 3) != 0) {
				throw new DataSetException("The number of columns for type XYEXYE " + 
			colCount + " is not divisible by 3.");
			}
		...
		case XYEEXYEE: 
			//repeated x,y,xerr, yerr. Number of columns should be divisible by four
			
			_hasXErrors = true;
			if ((colCount % 4) != 0) {
				throw new DataSetException("The number of columns for type XYEEXYEE " + 
			colCount + " is not divisible by 4.");
			}
...
//		setColumnIdentifiers(colNames);
	...
	/**
	 * Get a curve from an index. E.g., f the index is 0
	 * it will return the 1st Y column. Y Columns are also known as "curves".
	...
	/**
	 * Get a x column from an index. E.g., f the index is 0
	 * it will return the 1st X column.
	 * @param index the index
	...
	/**
	 * Compute the "standard" standard deviation (divide variance by N)
	 * using an accurate one-pass method.
	 * @param x the data
