...
//	private static boolean _firstTime = true;
	...
	/**
	 * Sets the full path to the torus map. If this is set before
	 * a call to getMagneticFields, it will look here first;
	...
	/**
	 * Sets the full path to the solenoid map. If this is set before
	 * a call to getMagneticFields, it will look here first;
...
//		init(); //harmless if already inited
	...
	private static void getMagneticFields() {
		...
		if ((_torus == null) || (_solenoid == null)) {
			...
			File clasJlibDir = findDirectory(search.getPath(), "clasJlib", 3);

			if (clasJlibDir == null) {
				search = search.getParentFile().getParentFile();
				System.err.println("second try for clasJlib: "
						+ search.getPath());
				clasJlibDir = findDirectory(search.getPath(), "clasJlib", 3);
			}
...
//	}
	 ...
	 * @return the magnetic field menu
	 */
	public static JMenu getMagneticFieldMenu(boolean incRotatedField, boolean includeTestFields) {
////		init(); //harmless if already inited
		...
		_torusItem = createRadioMenuItem("Torus",  menu,  bg, (_activeField != null) && (_activeField == _torus), al); 
		_solenoidItem = createRadioMenuItem("Solenoid",  menu,  bg, (_activeField != null) && (_activeField == _solenoid), al); 
		_bothItem = createRadioMenuItem("Torus and Solenoid",  menu,  bg, (_activeField != null) && (_activeField == _compositeField), al); 
		...
		_zeroItem = createRadioMenuItem("No Field",  menu,  bg, (_activeField == null), al); 
		_uniformItem = createRadioMenuItem("Constant 5 Tesla Field", menu,
				bg, (_activeField != null) && (_activeField == _uniform),
				al);
		_perfectSolenoidItem = createRadioMenuItem("Perfect 5 Tesla Solenoid", menu,
				bg, (_activeField != null) && (_activeField == _perfectSolenoid),
				al);
		menu.addSeparator();
		_invertTorusItem = createCheckBoxMenuItem("Invert Torus", menu, isFieldInverted(FieldType.TORUS), il);
		_invertSolenoidItem = createCheckBoxMenuItem("Invert Solenoid", menu, isFieldInverted(FieldType.SOLENOID), il);
		...
		if (_solenoid != null) {
			...
			_scaleSolenoidPanel = new ScaleFieldPanel(FieldType.SOLENOID, "Solenoid");
...
//			System.err.println("x = " +  x + " z = " + z + "  Bt = " + Bt/10 + " Bl = " + result[2]/10);
//			
//			x = 5.1f;
//			getActiveField().field(x, 0f, z, result);
//			System.err.println("x = " +  x + " z = " + z + "  Bt = " + Bt/10 + " Bl = " + result[2]/10);
...
//			System.err.println("x = " +  x + " z = " + z + "  Bt = " + Bt/10 + " Bl = " + result[2]/10);
	...
	private static void handleStateChange(ItemEvent iev) {
		...
		if (source == _invertTorusItem){
			...
			if (_solenoid != null) {
				...
				System.err.println("Solenoid inverted: " + _solenoid.isInvertField());
			}			
		}
			...
			if (_solenoid != null) {
				...
				_scaleSolenoidPanel.setEnabled(_scaleSolenoidItem.isSelected());
				System.err.println("Solenoid scaled: " + _solenoid.isScaleField());
			}			
		}
	...
	private static JRadioButtonMenuItem createRadioMenuItem(String label, JMenu menu, ButtonGroup bg, boolean on, ActionListener al) {
	...
	private static JCheckBoxMenuItem createCheckBoxMenuItem(String label, JMenu menu, boolean on, ItemListener il) {
	...
	public static void main(String arg[]) {
		...
		//drawing canvas
		final MagneticFieldCanvas canvas1 = new MagneticFieldCanvas(-50, -350, 650, 700., MagneticFieldCanvas.CSType.XZ);
		JPanel magPanel1 = canvas1.getPanelWithStatus(500, 465);
		final MagneticFieldCanvas canvas2 = new MagneticFieldCanvas(-50, -350, 650, 700., MagneticFieldCanvas.CSType.YZ);
