...
public class MagneticFieldCanvas extends JComponent implements MouseListener, MouseMotionListener {
	...
	protected void setAffineTransforms() {
		...
		_localToWorld.concatenate(AffineTransform.getScaleInstance(scaleX,
				-scaleY));
		_localToWorld.concatenate(AffineTransform.getTranslateInstance(-bounds.x,
				-bounds.y));
	...
	protected void drawMagneticField(Graphics g, Rectangle bounds) {
		...
		for (int sx = bounds.x; sx < bounds.x + bounds.width; sx += w) {
			...
			for (int sy = bounds.y; sy < bounds.y + bounds.height; sy += h) {
				...
				switch (_cstype) {
				case XZ:
					MagneticFields.field((float)(wp.y), 0f, (float)(wp.x), result);
					break;
				case YZ:
					MagneticFields.field(0f, (float)(wp.y), (float)(wp.x), result);
					break;
				}
...
//		MagneticFields.field((float)(_workPoint.y), 0f, (float)(_workPoint.x), _workResult);
			...
			public void mouseMoved(MouseEvent me) {
				...
				case XZ:
					...
					s = String.format("  loc: ( %-4.2f,  0,  %-4.2f) Bmag %-4.2f T  B = (%-4.2f, %-4.2f,  %-4.2f)", 
							_workPoint.y, _workPoint.x, bmag/10, Bx/10, By/10, Bz/10);
					break;
				case YZ:
					MagneticFields.field(0f, (float)(_workPoint.y), (float)(_workPoint.x), _workResult);
					Bx = _workResult[0];
					By = _workResult[1];
					Bz = _workResult[2];
